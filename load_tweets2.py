...
def insert_tweet(connection,tweet):
    ...
    with connection.begin_nested():
        ...
        sql = sqlalchemy.sql.text('''
        INSERT INTO users
            (id_users, created_at,updated_at,url,friends_count,listed_count,favourites_count,statuses_count,protected,verified,screen_name,name,location,description,withheld_in_countries)
        VALUES
        (:id_users, :created_at, :updated_at, :url, :friends_count, :listed_count, :favourites_count, :statuses_count, :protected, :verified, :screen_name, :name, :location, :description, :withheld_in_countries)
        ON CONFLICT DO NOTHING
            ''')
        ...
        sql=sqlalchemy.sql.text(f'''
            INSERT INTO tweets (
                id_tweets, id_users, created_at, in_reply_to_status_id, in_reply_to_user_id, quoted_status_id, retweet_count, favorite_count, quote_count, withheld_copyright, withheld_in_countries, place_name, country_code, state_code, lang, text, source, geo)
            VALUES (
                :id_tweets, :id_users, :created_at, :in_reply_to_status_id, :in_reply_to_user_id, :quoted_status_id, :retweet_count, :favorite_count, :quote_count, :withheld_copyright, :withheld_in_countries, :place_name, :country_code, :state_code, :lang, :text, :source, {'ST_GeomFromText(:geo, 4326)' if geo_str else 'NULL'})
            ON CONFLICT DO NOTHING
        ''')
        ...
        for mention in mentions:
            ...
            sql=sqlalchemy.sql.text('''
                INSERT INTO tweet_mentions (id_users, id_tweets)
                VALUES (:id_users, :id_tweets)
                ON CONFLICT DO NOTHING
            ''')
            connection.execute(sql, {
                'id_users': mention['id'],
